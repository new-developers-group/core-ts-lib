...
export class DateFieldValidation extends ValidationHelper implements Validation {
  ...
  validate(input: unknown): Error {
    ...
    if (!_isValidDate(value)) {
      const message = isTruthy(this.options) ? `${this.fieldName} ${this.options.customMessage}` : 'Invalid Date';
