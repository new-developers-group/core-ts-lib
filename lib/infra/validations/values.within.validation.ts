...
export class ValuesWithinValidation extends ValidationHelper implements Validation {
  constructor(readonly fieldName: string, readonly values: unknown[], readonly options: ValidationOptions) {
    super(fieldName);
  }
  ...
  validate(input: unknown): Error {
    ...
    if (this.options.strictEquals) {
      if (!containsValue(this.values as string[], value, isCaseSensitive)) {
        validationErrorMessage = `${input[this.fieldName]} ${this.options.customMessage}`;
      }
